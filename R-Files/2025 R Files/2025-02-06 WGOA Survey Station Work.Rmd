...
packages <- c("tidyr","dplyr","lubridate","hms", "ggplot2", "ggmap", "lattice", "stringr", "data.table",    "tibble","readxl", "sf","maps", "mapdata", "mapplots", "mapview", "marmap", "Cairo", "ncdf4",
"oce", "here","reshape2", "viridis", "export", "rnaturalearth","rgdal","raster", 
...
AK_Shp_Path <- here("Data","Shape_files_Alaska_dcw")
map <- sf::st_read(dsn = AK_Shp_Path,
...
WGOAdata <- read.csv(here("Data","WGOA station_list_dougherty_2019_ProjInstrutions.csv")) %>% rename("LAT" = "Latitude.N.") %>% rename("LON"="Longitude..W.")
...
# get bathymetry data
b <- getNOAA.bathy(lon1 = min(GridStaData$LON),
                   lon2 = max(GridStaData$LON),
                   lat1 = max(GridStaData$LAT),
                   lat2 = min(GridStaData$LAT),
                   resolution = 1, keep = TRUE)

b = getNOAA.bathy(lon1 = -166, lon2 = -140, lat1 = 52, lat2 = 62, 
                  resolution = 15, keep = TRUE)
