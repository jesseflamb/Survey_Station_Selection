...
with the DAS we have left.  Variables that can be adjusted: Gear.Sampled per station and time taken to sample gear (lines 77-85), Ship Speed (line 87).  Lastly, station order matters in terms of distance (line 55).  
...
# Staions Remaining 
StaTDL<-c(1:9,28:24,38:44,59:53,68:179,180,213:208,216:220,228:225,234:238,271) 
...
ODsta <- Sta %>% select(Grid.ID,LAT,LON,Gear.Sampled,Stn_Order) %>% 
...
depart <- ymd_hm("2023-05-14 16:00") # pre-survey calculations, takes ~8 hours to get to STA 1 from DH
